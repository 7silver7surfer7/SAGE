...
contract SocialNFTLauncher is ReentrancyGuard {
    ...
    function createEdition(
        ...
        editions[edition] = Edition({ artist: msg.sender, priceWei: priceWei_, maxSupply: maxSupply_ });
        allEditions.push(edition);
        emit EditionCreated(edition, msg.sender, name_, symbol_, priceWei_, maxSupply_);
    }
    ...
    function mint(address edition) external payable nonReentrant returns (uint256 tokenId) {
        ...
        tokenId = SocialEditionNFT(edition).mintTo(msg.sender);
    ...
    function mintedOf(address edition) external view returns (uint256) {
        return SocialEditionNFT(edition).minted();
    }
